...
(() => {
  ...
  const classCards=()=>nav&&nav.classes&&data?nav.classes.index({classes:data.classes,pupils:data.pupils}):new Map();
  ...
  async function load(wanted){
    ...
    try{
      ...
      // Паралелката се пишува како во секое мени: ознака · раководител · опис, целиот ред на лебдење.
      $('maClass').innerHTML=nav&&nav.classes?nav.classes.optionsHtml(classCards(),'',{empty:'Сите',extra:[['__none','Без паралелка']]})
        :option('','Сите','')+option('__none','Без паралелка','')+data.classes.map(c=>option(c.label,c.label,'')).join('');
  ...
  function renderDetail(){
    ...
    if(tab==='pupils'){
      ...
      const classPicker=nav&&nav.classes
        ?`<label>Паралелка / група<select name="grade" data-class-picker>${nav.classes.optionsHtml(classCards(),r.grade||'',{empty:'Без локална паралелка'})}</select></label>`
        :select('Паралелка / група','grade',classOptions,r.grade||'');
      html+=select('Одделение на ученикот','oddelenie',{'':'Непотврдено',...Object.fromEntries(roman.map(x=>[x,x]))},r.oddelenie||'')+classPicker+select('Основен статус','enrollmentType',{internal:'Внатрешен',external:'Надворешен'},r.enrollment_type)+select('Програма','programme',programmes,r.programme)+select('Локална настава / поставеност','placement',placements,r.placement)+`</div><div class="ma-checks" style="margin-top:18px"><label><input name="boarding" type="checkbox" ${r.boarding?'checked':''}>Користи интернат</label><label><input name="active" type="checkbox" ${r.annual_active?'checked':''}>Активен во ${esc(year)}</label></div><p class="ma-hint">Интернатот е независен од програмата. Надворешен ученик добива локална паралелка само со експлицитна модифицирана програма, подготвителна група или опсервација. Исклучувањето од година не ја брише историјата.</p>`;
    }else{
