...
(() => {
  ...
  const roles={teacher:'Наставник',therapist:'Терапевт',specialist:'Стручен соработник',administration:'Администрација'};
  ...
  function renderList(){
    ...
    const matches=rows().filter(r=>{
      ...
      if(![r.name,r.grade,r.identifier].filter(Boolean).join(' ').toLocaleLowerCase('mk-MK').includes(query))return false;
      if(tab==='employees')return true;
  ...
  function renderDetail(){
    ...
    if(tab==='pupils'){
      ...
      html+=field('Идентификатор на вработен · незадолжително','identifier',r.identifier)+`</div><h3>Улоги во ${esc(year)}</h3><div class="ma-checks">${Object.entries(roles).map(([k,v])=>`<label><input type="checkbox" name="roles" value="${k}" ${employeeRoles(r).includes(k)?'checked':''}>${v}</label>`).join('')}</div><div class="ma-grid" style="margin-top:16px">${select('Основен наставнички профил','teacherKind',{odd:'Одделенски',pred:'Предметен'},r.teacher_kind||'pred')}</div><p class="ma-hint">Улогите не доделуваат пристап или кабинет. Наставникот може да има и класни и предметни задолженија преку „Паралелки / предмети“. Без избрани улоги вработениот е неактивен само за оваа година; записите и термините остануваат.</p>`;
    }
  ...
  async function savePerson(event){event.preventDefault();if(busy)return;
    ...
    else Object.assign(body,{identifier:f.get('identifier')||null,roles:f.getAll('roles'),teacherKind:f.get('teacherKind')});
  ...
  root.addEventListener('click',async e=>{
    ...
    if(b.dataset.maTab&&canDiscard()&&!busy){tab=b.dataset.maTab;selected=null;dirty=false;root.querySelectorAll('[data-ma-tab]').forEach(n=>n.setAttribute('aria-pressed',String(n===b)));['maKindLabel','maClassLabel','maGradeLabel','maTherapistLabel'].forEach(id=>$(id).hidden=tab==='employees');$('maAdd').textContent=tab==='pupils'?'＋ Додај ученик':'＋ Додај вработен';renderList();renderDetail();}
  ...
  $('maSearch').addEventListener('input',renderList);['maActive','maKind','maClass','maGrade','maTherapist'].forEach(id=>$(id).addEventListener('change',renderList));
