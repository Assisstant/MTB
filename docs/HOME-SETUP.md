...
With the work PC off, nothing at work is reachable. ZenPC1 therefore runs its
own independent PostgreSQL database and local API. The current procedure uses
verified pCloud snapshots and manual acceptance; it does not require both PCs
to be awake together.

Use the **MTB Database - Manual Sync** desktop shortcut. Export on the PC where
you finished working, wait for pCloud, then Compare on the other PC. Choose a
legacy JSON area for selective app data or accept the complete snapshot for an
exact database copy. Every import is manual and creates a safety dump.

The complete current procedure is in
[MANUAL-DB-SYNC.md](MANUAL-DB-SYNC.md).

The USB and browser JSON procedure below remains a recovery fallback.

---

## LEGACY PART A — Browser JSON recovery export
