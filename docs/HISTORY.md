# MTB — technical history

Moved out of `CLAUDE.md` / `AGENTS.md` on 23 Sep 2026, unchanged, because
those files had grown past the size an agent loads whole (235k characters
against a 150k limit) and were being read only in part. The rules, the traps,
the layout and the commands stayed there; this is the WHY behind each feature,
section by section, and the dated State notes. Nothing here was edited in the
move. Where the current state is needed, read `SOSTOJBA.md` first.
...
## Дали е сè поврзано — и каде намерно НЕ е (12 Sep 2026)

Прашање на сопственикот, и заслужува точен одговор наместо „да, иста база е".

**Една база, но не еден факт.** Екраните не се копии еден од друг: секој пишува
во своја табела и се среќаваат само таму каде што некој ги врзал.

| факт | табела | кој го пишува |
|---|---|---|
| распоред на кабинетите | `schedule_slots` | RasporediFusion (право во базата) |
| **неделата на дневникот** | `diary_schedule` | S-Dnevnik, преку својот документ |
| присуство | `attendance` | S-Dnevnik |
| распоред на настава | `lessons` | НаставаУреди / увоз / демо |
| списоците за годината | `*_years`, `student_enrollments` | Податоци / работниот простор |

**ДВЕ НЕДЕЛИ, не една, и ништо не копира меѓу нив.** Ова е одговорот на „ако
нешто се пополни во распоред, се рефлектира ли во S-Dnevnik": **не.**
`schedule_slots` е договорот со училиштето — кој кабинет кое дете кога го зема;
`diary_schedule` е личниот план на еден терапевт, а `attendance.slot_key` е
буквално `ден-позиција` во ТАА недела. Термин внесен во Распоред не создава
место за чекирање во дневникот, ниту обратно. Тоа не е пропуст што треба да се
„поправи" со копирање: копирањето би направило втор сопственик на ист факт
(правило 5) и првиот што зачувува би ја пребришал неделата на другиот.

**Мостот меѓу ученик и дневник е `students.sdnevnik_id`.** `GET /api/diary/*`
клучи по него, па ученик без тој број се гледа во Податоци и во распоредот, а
неговото присуство никогаш не стигнува до дневникот. Тоа е ТИВОК прекин: редот
е запишан, избројан е во табелите, и го нема на екранот каде што се бара.

**Зошто екраните не можат сами да го кажат ова.** `/api/teaching/crossing` ги
филтрира термините низ годишните списоци со обични `JOIN`-ови — точно за
цртање, и невидливо кога нешто недостига: термин чиј ученик не е запишан оваа
година не се пријавува, тој едноставно го нема. Истото важи за терапевт вон
списокот. Значи нешто надвор од екраните мора да гледа.

`npm run check:consistency` е тоа. Само чита — **нема `--apply` и не смее да
добие**: секој ред во него е или одлука на човек или промена во кодот, а
скрипта што „поправа" идентитет што не може да го провери е токму промашувањето
поради кое постојат правилата 2 и 5.

**Поделено на ГРЕШКИ и РАБОТА, и таа поделба е поентата.** Грешка е ред што не
може да биде точен (термин што никој не го гледа, присуство што не може да
стигне до дневникот, еден факт запишан двапати и различно). Работа е ред што е
точен и нецелосен — паралелка што никој уште не ја внел. Излезот е 1 само за
грешки: проверка што секој ден црвени поради заостаток е проверка што никој не
ја чита, и тогаш вистинските грешки се кријат внатре.

Осумте грешки што ги бара, секоја видена во вистински податоци или во преглед:
ученик со термин а не е на списокот, терапевт исто, неактивен ученик со термин,
присуство кај ученик без `sdnevnik_id`, `students.grade` наспроти
`student_enrollments.grade`, ист наставник запишан двапати (главни наспроти
мали букви — `teachers.name` е уникатно по ТОЧНИОТ стринг), истото за терапевт,
и годишен каталог кон ученик вон годината.

**Каталогот е ГОДИШЕН** (миграција 019) — првата верзија на проверката го
заборави тоа и лани запишан каталог ќе го пријавуваше како грешка секоја
година. Најдено со пуштање, не со читање.

**`--mask` постои зашто извештајот е следното нешто што се лепи некаде.** Истата
поука како `check-names.ts`. Стандардно пишува вистински имиња, зашто без нив
наодот не може да се поправи, и го кажува тоа во заглавјето.

Проверено против вистинска база со намерно расипано училиште: сите осум грешки
се јавуваат, сите се гаснат кога ќе се поправат (излез 0), маскирањето работи,
и празна година вели „нема внесен распоред" наместо да ги пријави сите
паралелки како проблем.

**Ширината на екранот е прозорецот, не 1700px (12 Sep 2026).**
`Nastava.html`, `NastavaUredi.html` и `Podatoci.html` имаа `main { max-width:
1700px }` (Податоци 1500). Кај мрежа од 22 наставници × 35 часа тоа не штити
ништо: таблицата и онака е поширока, па капата само додаваше празна маргина
покрај нешто што и понатаму се лизга — зголемувањето на прозорецот не менуваше
ништо. Сега страницата го зема прозорецот, а капата останува САМО врз текстот
(`105ch`), зашто читливата ширина за тоа и служеше.

Измерено во прелистувач на 2000px: `main` е 2000, објаснувачкиот текст ~1000,
и телото нема страничен скрол — ни на 400px, каде гутерот останува.

## Распоредот на кабинетите има свој влез (12 Sep 2026)

Наставата се внесуваше од работна книга од првиот ден; распоредот на кабинетите
немаше таков пат — само рака по рака во Fusion. А центарот го води во своја
табела: колона по КАБИНЕТ, надолу ден и часовите I…VI, во ќелијата детето.

`npm run import:cabinets` го чита тој лист. Поделено на три, како и кај
наставата: `lib/cabinet-sheet.ts` е чист (решетка стрингови внатре, наоди
надвор — па се тестира на центар што не постои), `lib/cabinet-import.ts` го
решава она што е прашање за ГОДИНАТА, а скриптата е работната книга, знамињата
и извештајот.

**Пишува преку `PUT /api/schedule/block`, никогаш со свој INSERT.** Тој
endpoint веќе е сопственик на еден блок од кабинетската недела: одбива
терапевт закажан двапати во ист час, одбива дете на две места, сам го дели
блокот на две дваесетминутни половини и го серијализира денот на терапевтот.
Втор запишувач без ништо од тоа е грешката за која овој проект постојано плаќа.
Затоа серверот мора да работи, како и кај алатките за список и дневник.

**Четири работи не ги погодува**, зашто секоја од нив измислува третман:

- **Кој кабинет е чиј.** Колоната вели „Логопед", а `schedule_slots` бара
  терапевт. Се решава по ред: `--map` (човек го кажал), па `specialist_categories`
  преку `therapist_years.category_id` — категорија што ја држи ТОЧНО ЕДНО лице.
  Скратено име („Сензорна" наспроти „Сензорна интеграција") се прифаќа само ако
  е почеток на точно една категорија со точно еден држач, и **се пишува во
  извештајот**: изведување што никој не го гледа е погодување.
- **Бојата.** Ќелиите во вистинскиот лист се обоени во неколку бои и никој не
  кажал што значат. Решетка од стрингови не гледа боја — тоа е искрената
  верзија на „не знам".
- **Повеќе од две имиња во ќелија.** Блок се дели на две половини и не подалеку.
  Три имиња е или група — која овој распоред сè уште не ја изразува — или
  печатна грешка.
- **Кое дете е тоа име.** Точно совпаѓање, па `bareName`; две деца со тоа име →
  се пријавува и се испушта (правило 2). Не создава ученици: име што го нема на
  годишниот список се пријавува и неговиот термин останува надвор.

**Каталогот го најде пуштањето, не читањето.** `PUT /api/schedule/block` одбива
дете што не е во каталогот на тој терапевт — точно, зашто ќелија во распоред не
е одлука за тоа со кого работи еден терапевт. Без тоа увозот ќе враќаше двесте
идентични одбивања. Сега планот ги ИМЕНУВА тие парови пред запишување, а
`--caseload` ги запишува преку `PUT /api/therapists/:name/students/:publicId` —
endpoint-от што го поседува тој факт, истиот што го праќа чекбоксот во Fusion.
Одбрано зборче, не тивка последица.

Проверено против вистинска база и жив сервер: `npm run test:cabinets`, 21
тврдење — вклучувајќи дека без каталог серверот ги одбива СИТЕ, дека по
поврзувањето ги прифаќа сите, дека второ пуштање заменува наместо да дуплира, и
дека соименик и непознато име не влегуваат. Читањето на .xlsx е единственото
што не е покриено овде: `xlsx` е tarball од CDN што не се инсталира во секоја
средина, па поделбата е на РЕШЕТКАТА — четирите реда над неа се истите што
`import-teaching.ts` ги користи во работа.

## Приближно име: се нуди, не се одлучува (12 Sep 2026)

Сопственикот го кажа очекувањето: „да ги споредиш и интерпретираш имињата
приближно" — и да не прифаќа кога не е сигурно за кое дете се работи. Тоа се
две работи и втората ја одредува првата.

`lib/name-match.ts` е приближната споредба. Точниот пат (`bareName` + мали
букви) останува тоа што ОДЛУЧУВА; ова е другата половина, за кога ништо не
совпаднало точно: кажува кои имиња се БЛИСКИ, за да гледа човек три кандидати
наместо список од осумдесет.

**Не одлучува ништо.** Правило 2 не е „споредувај внимателно" туку „никогаш не
погодувај совпаѓање на идентитет" — а блиско име Е погодување колку и да е
добра аритметиката. Во ова училиште две деца делат едно име; трето и четврто
што би се разликувале за една буква не би ги разликувал ниту еден растојание на
свет. Затоа сè враќа КАНДИДАТИ, а кандидат станува идентитет само кога човек го
запише еднаш.
...
**Растојанието е помалото од две читања, и обете се потребни — измерено, не
претпоставено.** Сортирањето на зборовите е тоа што прави „Бошевски Михаил"
исто име; истото сортирање го расипува испуштената ПРВА буква, зашто „ошевски"
се реди по „михаил" а „бошевски" пред него, па сортираните низи се разликуваат
за многу повеќе од една буква. „Михаил ошевски" е вистинска ќелија во работната
книга на центарот, па читањето по редослед мора да остане.

**`--names` е човечката одлука, зачувана.** Обична датотека
`{ "спелување во листот": "RS-123" }` (или точното име од базата — се чува
id-то, па врската преживува подоцнежно преименување). Извештајот ја печати
линијата готова за копирање, па читањето што човек го прави еднаш не се бара
пак при секој увоз. **И не е заобиколување на правило 2**: ако наведеното име
го носат две деца, се одбива и бара `public_id`.

`test/name-match.test.ts` е 11 тврдења врз луѓе што не постојат — испуштена
буква, тастатура оставена на латиница, „ѓ" куцано како „г", презиме прво,
половина име (што намерно НЕ е блиско: половина име не е доказ), и дека два
кандидата на еднакво растојание се изговараат како неодлучени. `test:cabinets`
порасна на 28: дека блискиот промашок се пријавува и НЕ станува термин, дека
најблиското вистинско име се нуди со својот `public_id`, и дека `--names`
решава — освен кај соименици.

## Два застарени теста, најдени со пуштање (12 Sep 2026)

Кога `NastavaUredi.html` доби свое јазиче во лентата, `app-navigation.browser.mjs`
остана да очекува пет. Десет тврдења беа црвени за промена што беше НАМЕРНА —
облик на застарено очекување, не дефект, а пакет што е црвен поради нешто по
кое никој не постапува престанува да се чита воопшто.

Под неа стоеше вториот, поинтересен: **една константа стоеше за два различни
списоци.** `start.html` има своја поделба и намерно го води „Уреди настава" меѓу
алатките, не меѓу картичките — па лансерот покажува една картичка помалку
отколку што лентата покажува јазичиња. Поправањето на првото го поцрвени
тврдењето за лансерот, за причина што нема никаква врска со лансерот.
`LAUNCHER_CARDS` сега е свој број. 76 тврдења, зелено.

**И рамката се вчитуваше двапати.** `repointFrames()` споредува со
`appSrc()`, кој секогаш додава `?embed=1`; почетниот `src` во HTML-от го немаше,
па дури и кога школката е послужена ОД истиот сервер рамката се вчитуваше
еднаш, па повторно. Измерено во прелистувач: **2 барања пред, 1 по**
поправката — `src="RasporediFusion.html?embed=1"`.

## „Не постои во распоредот" беше една реченица за две работи (12 Sep 2026)

Сопственикот го праша најдиректно: „II-а одделение не постои — имам за." И беше
во право. Црвената картичка велеше „одделението „II-а" не постои во распоредот
на настава", а тоа е точно и бескорисно: се чита како дефект во записот на
детето, додека вообичаената вистина е спротивната — паралелката е формирана, на
списокот е, и распоредот за неа едноставно не е внесен.

Затоа `/api/teaching/crossing` сега разликува два `reasonCode`, зашто тоа се две
различни работи за човек:

| `class-not-timetabled` | е на списокот, нема ниту еден час → внеси во Уреди распоред |
| `unknown-class` | не е ни во распоредот, ни на списокот → провери во Податоци |

Одлуката ја носи серверот, не страницата: тој веќе ги има и двата списока
(`classRows` е годишниот список, `known` се ознаките што распоредот ги именува),
а втора пресметка во прелистувачот е точно она за што овој проект плаќа.

**Што НАМЕРНО не се нуди тука: најблиската друга ознака.** „II-б" е една буква
од „II-а" и е друга просторија со други деца. Кај ИМЕ приближното совпаѓање е
кандидат што човек го суди (`lib/name-match.ts`); кај ОЗНАКА НА ПАРАЛЕЛКА тоа е
покана да се стави дете таму каде што никогаш не било (правило 2). Истата
машинерија, спротивна одлука — и затоа е напишано овде, за да не се „доработи"
подоцна.

**И списокот сега води со РАБОТА, не со редови.** Дваесет реда што сите велат
„II-а нема часови" се една работа, не дваесет, а заостаток што се чита како
дваесет е заостаток што никој не го започнува. Групирано по парот (причина,
паралелка) — по парот, зашто иста паралелка може да е неповрзана од две различни
причини и спојувањето би ја сокрило токму онаа што се поправа.

## Шестиот приказ: „Ден по ден" (12 Sep 2026)

Побарано: сите денови еден под друг. „Цела недела" ги реди деновите НАПРЕЧНО и
таа мрежа е поширока од секој екран — пет дена по седум часа секогаш ќе се лизга
странично. Ова е истата недела читана НАДОЛУ: по еден блок за секој ден, а
внатре мрежата што сите веќе ја читаат („По одделение"). Единствениот од двата
што се печати без да се исече — секој ден на своја страница.

**Една копија на мрежата, два повикувача.** `classGridHtml(cells, dayKey)` е
изнесена од `drawByClass`; втора копија од тој HTML е како двата приказа би
почнале да не се согласуваат за една боја или еден клуч.

**Клучот на ќелијата го носи ДЕНОТ**, како кај неделната мрежа и од иста
причина: истата паралелка и час постојат пет пати, па клуч без ден отвора
понеделник од ќелија во среда — и изгледа сосема веродостојно.

**И панелот на клик сега го именува денот во СЕКОЈ приказ што држи повеќе од
еден.** Беше врзан за `view === 'week'` по име, па новиот приказ го заборави
истиот пропуст; сега е врзан за `wholeWeek()`. Најдено во прелистувач — панелот
велеше „I-а · 2. час" за еден од пет часа и не кажуваше кој.

Истото барање како неделната мрежа, па префрлањето меѓу нив не праќа ново
барање. Тестот го тврди тоа, плус дека секој блок носи свој ден во клучот.

## Одделението кажува КОИ, не само колку (12 Sep 2026)

Барање на сопственикот: во „Одделенија" да пишува кои ученици се во одделението,
да има dropdown со кој се ставаат од списокот, и — пошироко — **каде што се
покажуваат податоци, да има врска до местото каде што се менуваат.**

Табелата дотогаш одговараше „колку". Тоа е единственото прашање за едно
одделение на кое никому не му треба помош: човекот што ја формира годината
прашува КОИ деца, а тој одговор стоеше едно јазиче подалеку, без пат до него.

**Ставањето во одделение е ЕДЕН запис, не два.** Ученикот држи едно одделение
по учебна година — тоа е една колона на запишувањето — па ставањето овде само
го вади од претходното. Затоа панелот намерно НЕМА чекор „тргни од старото":
таков факт не постои за да се тргне. Ако некогаш се појави втор запис што
„го вади", тоа е втор сопственик на еден факт (правило 5) и двата ќе се разидат
првиот пат кога едниот ќе падне.

**Се пишува преку `PATCH /api/students`, и се праќа САМО одделението.** Тој
endpoint веќе го поседува одделението на ученикот; втор запишувач е грешката за
која овој проект постојано плаќа. Не се праќа име — па овој екран не може да
препише спелување што некој го исправил на друго место, истата стапица како во
работниот простор — ниту вид, на кој се одговара во „Ученици".

**Врската од покажаното кон местото каде се менува.** Чипот со ученик отвора
„Ученици" и го обележува неговиот ред; чипот со раководителот отвора
„Наставници". Пребарувањето и филтерот се празнат пред префрлањето: ученик
скриен зад филтер би го спуштил човекот на јазиче каде редот што го кликнал
воопшто не е нацртан, што се чита како расипана врска, а не како филтер што
останал вклучен.

**А серверот врати HTTP 500 со името на ограничувањето.** Миграцијата 029 вели
дека активен интерен или интернатски ученик мора да има одделение. Тоа правило
се среќаваше преку обичен `UPDATE`, па бришењето на одделението враќаше
`kind_matches_grade` под 500 — што се чита како расипан endpoint, а не како
правило, и токму овој екран е местото каде човек го среќава. Сега е 409 со
`needsClass`, страницата го кажува во една реченица на македонски, а во менито
„премести во…" интерен ученик воопшто не добива „без одделение". Екстерен и
понатаму смее да нема одделение — тоа е половината што 029 ја исправи кај 028.

Проверено со `npm run test:podatoci-classes` (20 тврдења во вистински
прелистувач, во учебна година своја, секое запишување прочитано од базата, без
слика — оваа страница листа вистински деца). Двете тврдења за 409 беа проверени
дека ПАДААТ прво: без заштитата во `roster-write.ts` враќаат 500.

## Потврда дека е ВО базата, кажана во мирување (12 Sep 2026)

Сопственикот го кажа најдиректно: „ако еднаш внесам, да се пази, да не се
враќам по 10 пати". Внесуваше профил/кабинет на деветте терапевти и не беше
сигурен дали останало.

**Останувало. Проблемот беше што екранот не го кажуваше.** Изборот се зачувува
на `change` и тоа се јавуваше со toast што исчезнува за 2.6 секунди — па десет
секунди подоцна редот не тврдеше ништо. Единствениот чесен одговор на екран што
молчи е да го внесеш пак, што точно и се случуваше.

Сега ќелијата го носи **зачуваното** состојбено: „✓ во базата" или „не е
поставен", изведено од `data.categoryOf` при СЕКОЕ прецртување. Затоа доказот е
F5: ако по освежување сè уште пишува „во базата", тоа е читање од базата, а не
спомен на кликот. Тестот го тврди токму така — сетирање право во
`therapist_years`, па reload, па читање на екранот.

И зачувувањето сега го зема она што **серверот вели дека запишал**
(`answer.categoryId`), не она што табот го побарал; одбиено зачувување го враќа
и маркерот, не само менито, инаку ќелијата останува да тврди состојба што базата
никогаш не ја примила.

**Наставникот нема профил/кабинет, и колоната е тргната.** Беше празна кај сите
22 и ќе останеше празна: во ова училиште профил држи СТРУЧНОТО лице, а човекот
што го има и двете го држи профилот како ТЕРАПЕВТ — друг ред, друго јазиче.
Колона што може само да биде празна е токму „поле без сопственик" што овој
проект го одбива на друго место. Шемата и `/api/categories/holder` се
недопрени: `teacher_years.category_id` останува, само не се внесува од тој
екран.

**Предметот се нуди од каталогот на МОН и тука.** Беше голо поле за пишување
додека `teaching_subjects` (миграција 032) веќе го нудеше во НаставаУреди и во
работниот простор. Прво како `datalist`, а од истиот ден како ВИДЛИВ избирач
со чипови — види „Еден наставник, повеќе предмети" подолу, каде е и причината
поради која `datalist` не беше доволно. И во двете верзии **понуда, не
ограничување**: `teachers.subject` останува слободен текст и работната книга
пишува кратенки (`ФЗО.`) што серверот ги разбира преку `SUBJECT_ALIASES`.
Зачуваното што каталогот не го знае се задржува, за да не изгледа како грешка
што треба да се поправи.

Шест нови тврдења во `npm run test:podatoci` (67 вкупно), во вистински
прелистувач, со читање од базата.

## Комбинираната паралелка веќе има одговор — само не се гледаше (12 Sep 2026)

Сопственикот побара „мапата за одделенијата како во вордот" и праша дали е
изгубена. **Не постоела како документ.** Она што постои е поделбата на два
факта, и таа е одговорот:

| ПАРАЛЕЛКА (`student_enrollments.grade`) | каде учи детето |
| ОДДЕЛЕНИЕ (`student_enrollments.oddelenie`) | која генерација е |

Кај единаесетте нумерирани паралелки двете се исти. Кај петте комбинирани не
се — дете во „комбинирана II, III, IV" учи во ЕДНА просторија и е во ЕДНА од
три генерации. Тоа е точно, не грешка.

Миграциите 030/031 го создадоа `oddelenie` и го именуваа за сопственик на тој
факт, а работниот простор го уредуваше од 12 септември. **`Podatoci.html` не го
покажуваше воопшто** — а тоа е екранот на кој се формира годината. Значи
механизмот постоеше, а на местото каде што се работи комбинираната паралелка
изгледаше како обична и разликата немаше каде да се види.

Сега „Ученици" ги носи двете полиња едно до друго, именувани со тоа што значат
(„Паралелка · каде учи", „Одделение · генерација"), а во „Одделенија" чипот со
детето ја пишува генерацијата **само кога кажува нешто ново** — кај нумерирана
паралелка тоа би било двапати иста работа, а разликата што е важна би се
изгубила во неа. `romanOf` е истата функција како во школката.

**Што и понатаму НЕ е решено, и зошто не е итно.** `school_classes.label` е
уникатен текст, па пет реда „комбинирана паралелка" не можат да постојат. Но
тоа не е ѕид: §8.2 и онака ги нумерира („I комб.", „II комб."), а такви ознаки
се уникатни и се внесуваат како секоја друга. Со генерацијата запишана под нив,
ознаката повеќе не мора да ја носи целата вистина. Вистинскиот модел —
паралелка со ВИД и со МНОЖЕСТВО генерации — останува отворен, и правилото од
„Кратенките во името" важи: не се гради пред работата да го произведе
конфликтот.

**И две ситници од истиот екран.** Чипот во темна тема користеше `--soft`, само
една нијанса над `--card`, па име на дете се губеше во позадината; сега има свој
`--chip`. Лизгачот за големина беше `step="5"` — скокаше; сега е по 1%.

Шест тврдења повеќе не се додадени: `test:podatoci` (67) и
`test:podatoci-classes` (20) се пуштени непроменети и остануваат зелени, а
самата промена е проверена во прелистувач со дете чија генерација се разликува
од ознаката.

## Еден наставник, повеќе предмети (12 Sep 2026)

Сопственикот го кажа при внесување на вистинската година: „ако ги знаеш
предметите да има паѓачко мени за бирање по наставник… и внимавај некои
предаваат повеќе предмети."

Дотогаш „Предмет" беше **голо поле за пишување во трите екрана** што го
внесуваат — НаставаУреди, Податоци и работниот простор. Каталогот на МОН
(`teaching_subjects`, миграција 032) стоеше во базата од 12 септември и се
нудеше само во ќелиите на распоредот; кај наставникот го немаше воопшто во
НаставаУреди, а во другите два беше `datalist`. **`datalist` е понуда што не се
гледа**: се појавува дури кога ќе почнеш да го пишуваш токму она што го бараш,
па човек што не знае како точно се вика предметот не дознава дека воопшто
постои список.

**ЕДНА КОЛОНА, СПИСОК ВО НЕА.** `teachers.subject` останува слободен текст, а
повеќе предмети се пишуваат разделени со запирка. Тоа е одлука, не леност:

- Она што ја чита колоната како ФАКТ е точно **една** функција —
  `teacherHandles` во `lib/teaching-demo.ts`. Сѐ друго само ја печати. Таа сега
  го дели списокот и точно е кога **кој било** член совпаѓа.
- Табела би барала миграција, увозот од работната книга и трите екрана да се
  преместат **одеднаш** — обликот што овој проект веќе го измери како цена на
  една изгубена сесија (види „A schema cutover was attempted").
- Проверено пред да се одбере разделувачот: **ниту еден од 39-те предмети во
  каталогот не содржи запирка**, па делењето не може да пресече име на предмет.

**И кога ќе биде време за табела, тоа ќе се препознае:** кога предметот ќе
почне да носи СВОЈ факт — фонд часови, наставен план, во која паралелка. Тој ден
списокот во колона престанува да стигнува. Не порано.

**Тивката грешка што ја немаше кој да ја види.** Пред делењето, наставник
запишан како „ФЗО., ЛИК." не совпаѓаше со **ниту еден** предмет: целиот стринг
одеше низ `SUBJECT_ALIASES` како едно спелување, не наоѓаше ништо, и човекот
беше прескокнуван за секој час по своите предмети — додека генерираната недела и
понатаму изгледаше сосема веродостојно. `teaching-demo.test.ts` го тврди тоа
сега, и беше проверено дека ПАЃА против старата функција.

**Избирачот е ЕДНА копија, во `app-navigation.js`.** Истиот во трите екрана:
чипови за избраното, „+ предмет…" со каталогот, и „✎ друго…" за слободен текст
— зашто работната книга пишува кратенки (`ФЗО.`) што серверот ги разбира преку
`SUBJECT_ALIASES`, и еден избор од мени не смее тивко да ги замени. Зачуваното
што каталогот не го знае се ЗАДРЖУВА како чип. Секој екран веќе имаше своја
копија на каталогот; сега ја нема ниту една.

Тој остава **скриено поле со истата класа/id** (`.t-subject`, `#tSubject`) внатре
во ќелијата, па патот на зачувување во трите страници не е сменет воопшто —
сменето е само како човек внесува. Зато и праќа `input` и `change`, за да знае
редот дека има непратена измена.

**И се затвори уште едно тивко бришење, во Податоци.** `syncTeacherSubject` го
празнеше полето штом улогата е „одделенски" — при **секое** цртање, не само при
менување. Значи: отвораш одделенски наставник на кого работната книга му
запишала предмет, полето е празно, притискаш „Зачувај податоци" за ИМЕТО — и
предметот е избришан од базата. Точно истата форма што веќе беше затворена во
работниот простор и оставена отворена тука. Сега Податоци ги одлучува трите
случаи намерно, како и работниот простор: **предметен** → тоа што пишува;
**штотуку стана одделенски** → `null`, а редот го кажува тоа однапред („со
зачувување се брише"); **веќе беше одделенски** → полето воопшто не се праќа.
Серверот ги разликува отсутно и `null` од почеток (`PUT
/api/teaching/teacher/:id` гради SET список наместо COALESCE).

**`markSubmittable` сега слуша на РЕДОТ, не по поле.** Избирачот се прецртува
кога ќе се смени улогата, па полето врзано при првото цртање повеќе не постои —
и редот молчеше дека има непратена измена. Категоријата останува изземена:
таа се зачувува сама.

**Границата во API-то порасна од 120 на 400 знаци, само кај наставникот.**
Четири од подолгите имиња во каталогот поминуваат 120 заедно. `lessons.subject`
останува 120: една ќелија држи точно еден предмет.

Проверено: `test:podatoci` (со три нови тврдења, и она за бришењето проверено
дека ПАЃА прво), `test:uredi`, `test:podatoci-classes`, `test:navigation`,
...
## Едно поврзано чувство, не шест одвоени страници (12 Sep 2026)

Сопственикот го кажа директно: екраните треба да се чувствуваат како ЕДНА
апликација — S-Dnevnik така е правен, од повеќе мали склопени во еден систем.
Три работи го кршеа тоа, и сите три беа поправливи со она што веќе постоеше.

**`NastavaUredi.html` не беше во заедничката лента.** Ја вчитуваше
`app-navigation.js`, но не беше во `APPS`, па кога си на неа ниту едно јазиче
не е активно и страницата виси надвор од семејството. Сега е јазиче како
другите, а двете страници се врзани и директно: „✏️ Уреди распоред" од
прегледот, „👁️ Преглед со терапии" назад. Иста работа во два режима, а не
две апликации.

**Изборот на сервер живееше само во `Podatoci.html` и `start.html`.** Целата
машинерија — `SERVER_DEFAULTS`, `configuredServers`, `probeServer`,
`rememberServer` — беше напишана и проверена, само заклучена во една страница.
Преместена е во `app-navigation.js`, датотеката што сите поврзани екрани веќе ја
вчитуваат, со ИСТИТЕ клучеви (`mtb_servers_v1`, `mtb_podatoci_server_v1`).
Чипот БАЗА сега е копче: скенира кој сервер одговара (● / ○) и те префрла.
Копирањето во трета и четврта страница беше очигледната алтернатива и токму тоа
е грешката за која овој проект постојано плаќа.

**Настава и НаставаУреди пишуваа на гола релативна патека**, па отворени од
GitHub Pages беа мртви. Сега поминуваат низ `MTBAppNavigation.apiBase()`.

**Зошто ова НЕ го крши правилото „не додавај пронаоѓање сервер во апликациите".**
Тоа правило ги штити апликациите што држат СВОЈА КОПИЈА и се синхронизираат —
`S-Dnevnik.html` и `Rasporedi.html`, кои `app-navigation.js` веќе ги држи во
`LOCAL_FIRST`. Таму еден прелистувач што зборува со две бази го произведува
застојот „и двете страни се сменија". Настава, НаставаУреди и Податоци не држат
ништо: пишуваат право во базата и известуваат успех дури откако endpoint-от
прифатил. Немаат watermark, па немаат ни разидување.

Двете заштити остануваат: изборот е ЕДЕН клуч за целиот пакет, па два екрана не
можат да гледаат во две бази истовремено; и на origin на сервер менувањето база
НАВИГИРА кон истата страница на другата машина, наместо тивко да го пренасочи
отворениот таб. Origin-от и понатаму значи една база.

**Предметот се избира од каталогот на МОН (12 Sep 2026).** Во неделниот
приказ ќелијата има два реда: горниот е одделението, долниот е предметот.
Секој отвора свој избирач. Брзиот внес останува брз — одделението се запишува
со еден клик како порано, предметот е втор, доброволен чекор.

Серверот **веќе го примаше** `subject` во `PUT /api/teaching/teacher-lesson`;
недостигаше само понуда и избирач. Миграција 032 ја додава таа понуда како
`teaching_subjects`, изнесена од работната книга на МОН/БРО за 2026/2027 —
шест наставни плана (редовен плус петте посебни што овој центар ги користи),
40 различни предмети, 707 реда предмет × одделение.

**Понуда, не ограничување.** `lessons.subject` останува слободен текст и
работната книга на училиштето и понатаму пишува во него што ќе најде. Предмет
надвор од каталогот се зачувува како и порано, а веќе зачуван предмет што
каталогот не го знае се задржува и се прикажува во менито — инаку еден клик
би го избришал тивко.

**УНИЈА од одделенијата, не највисокото.** Сопственикот предложи кај
комбинирана паралелка да се земе највисокото одделение „бидејќи веројатно ги
има сите". Измерено против работната книга тоа не важи: кај Аутизам,
Мултихендикеп и Интелектуална попреченост деветто носи 11 од 25 предмети, па
тоа правило би сокрило повеќе од половина токму во плановите што овде се
најчести. Затоа `GET /api/teaching/subjects` зема унија.

**Кои одделенија — и ова е првиот читател на `oddelenie`.** Редоследот е:
`student_enrollments.oddelenie` (фактот што миграција 030 го создаде и 031 го
именуваше како сопственик на опсегот генерации) → римскиот број во ознаката на
паралелката → сè. Одговорот носи `basis`, па менито во насловот кажува по што
стеснило. Кога не може да утврди, нуди сè: четириесет предмети се мала
незгода, сокриен предмет што му треба на наставникот е ѕид.

**Планот НЕ се изведува од описот на паралелката.** `class_years.description`
вели „ученици со оштетен слух" со зборовите на училиштето, а миграција 031
изречно вели дека ништо не смее да пресметува од неа. Затоа сите планови се
нудат заедно, обединети по име на предмет.

**Вкрстувањето сега се чита и од страната на кабинетот (12 Sep 2026).**
Сопственикот ги именуваше двете половини на истото прашање: наставникот сака
да знае кој му се губи од часот, а кабинетот сака да знае кого зема во својот
термин и КАДЕ е тоа дете — чиј час, кое одделение. Првата половина ја имаше
одговорено од почеток; втората немаше екран воопшто, иако одговорот беше во
истиот одговор на API-то цело време.

„🧩 По кабинет" во `Nastava.html` е четвртиот приказ: редови се терапевтите,
колоните се часовите, а ќелијата го кажува детето и под него каде да го најде
— одделение и наставник.

**Прегрупирање, не второ пресметување.** Во кој час паѓа еден третман и за
колку минути се одлучува еднаш, во `lib/crossing.ts`; овој приказ само го чита
тој одговор од другиот крај. Второ мислење тука би можело да не се согласи со
екранот на наставникот за истото дете, а тоа е грешката за која овој проект
постојано плаќа.

**Терапевт чиј третман не може да се смести пак добива ред** — празен, но
...
## Редоследот на списоците, и профилот што значи „не е наставник" (22 Sep 2026)

Две барања од сопственикот при внесување на вистинската година, и обете се
покажаа како нешто што шемата речиси го имаше.

### „Основен наставнички профил" мораше да се избере и кога човекот не е наставник

Менито нудеше само **одделенски** и **предметен**, а тоа се двата начина на кои
се чита распоредот на наставата — кај одделенска редот е паралелката и ќелијата
е предметот, кај предметна обратно. Вработен во кабинет или во стручна служба не
е ниту едното, па формата го тераше да избере нешто невистинито за да може да
зачува.

**Третата вредност не е трет вид наставник — таа е ОТСУСТВО на наставнички
профил.** Тоа веќе беше изразливо во базата: без улога „Учествува во настава"
воопшто не се создава ред во `teachers`. Значи ништо не се додаваше во шемата;
менито само почна да го кажува тоа што записот и онака го значеше. Затоа нема
миграција за ова: `teachers.kind` останува `odd`/`pred`, зашто ред во таа табела
е наставник по дефиниција.

Во другата насока се одбива изречно: **наставник во распоред не смее да биде
ниту едното.** CHECK-от во базата и онака го одбиваше — со HTTP 400 што вели
„избраните податоци не се усогласени", реченица по која никој не може да постапи.
Сега endpoint-от го кажува што да се направи, а тестот го тврди ТЕКСТОТ, не само
статусот: проверено дека паѓа без заштитата.

И екранот го кажува **пред** зачувување, во белешка до менито, а никогаш не го
презапишува изборот. Тоа е истата стапица што веќе е платена двапати во овој
проект (`sync()` што го празнеше предметот при секое цртање): поле што само се
враќа на нешто додека човек го гледа е како се брише податок со притискање
„Зачувај" наменето за нешто сосема друго.

Новододаден вработен сега почнува од „Не е наставник" наместо од „Предметен" —
претпоставка помалку.

### Стрелки за поместување редови, и зошто редоследот е ПОДАТОК

Барање: „листите е добро да имаат стрелки за поместување на редовите;
нумерацијата на редот останува, се поместуваат содржините на редовите" — со
конкретен пример кај комбинираните паралелки, каде едно лице треба да стои точно
под друго.

Четирите списока во `Podatoci.html` (ученици, наставници, терапевти, одделенија)
се читаа по азбука, по ознака или по `sort_key`. **Годишната програма не се чита
така**: таа ги реди луѓето по оддели, а комбинираната паралелка стои до
наставникот што ја држи. Значи бараниот редослед не е ништо што читач може да го
пресмета — тој е одлука, и како секоја одлука има сопственик и место.

**Во базата, годишно.** `roster_order` (миграција 038) чува само позиција:
(учебна година, список, клуч, позиција). Три причини, секоја платена на друго
место во овој проект:

- **Не во прелистувачот.** Оваа страница намерно не памети ништо локално и
  пакетот го тврди тоа; а редослед запаметен на ДОМА е редослед што РАБОТА го
  демантира.
- **Годишно**, зашто списоците се внесуваат наново секој септември и архивската
  година мора да продолжи да се чита како тогаш. Нова година почнува од
  редоследот на читачот, не од ланската распоредба на други луѓе.
- **Без странски клуч, и намерно.** Четирите списока се клучат различно —
  ученик по `public_id`, другите три по нивниот број — а една колона не може да
  покажува кон четири табели. Тука не се одлучува ниту членство, ниту идентитет,
  ниту пристап: клуч што не именува никого едноставно не совпаѓа, и исчезнува
  при следното поместување, зашто списокот се запишува цел.

**ОТСУТНО значи „уште не е наместен", не „прв".** Некој додаден по наместувањето
се чита на крај додека човек не го помести — видливо, а не тивко пренаредување
на она што веќе било средено.

**ЦЕЛ СПИСОК, не еден ред — спротивно од ќелија во распоред, и намерно.** Едно
поместување менува најмалку две места, еден човек наместува еден список на еден
екран, и „очекувано" за редослед не значи ништо. Што може стар таб — да ги
намести оние што ги знае и да остави новајлија ненаместен, што читачот го реди
на крај и човек го помести еднаш. Не може да отстрани никого, да додаде никого,
да смени име ниту да допре членство.

**Клучевите НЕ се проверуваат наспроти годишното членство**, исто така намерно:
таа проверка би била втора копија на четирите филтри во `/api/roster`, а двете
би се разишле. Ред што именува никого чини еден неупотребен ред.

**Еден сопственик, повеќе читачи.** Стрелките се само во „Податоци", а
наместувањето го чита `/api/roster` — значи го следат и работниот простор, и
колоните на терапевтите во Fusion, и S-Дневник. Втора пресметка во некој од нив
е точно грешката за која овој проект постојано плаќа.

**Соседот е следниот ВИДЛИВ ред.** Кај учениците списокот е филтриран, па
„нагоре" значи „над оној што го гледам над себе"; скриените редови остануваат
каде што биле, зашто човекот не одлучувал за нив.

**Поместувањето одбива кога има незачуван ред.** Тоа препрочитува сè од базата,
па незачувана измена во друг ред би се фрлила тивко.

**Се применува во ЕДНО место, по читањата, не во четирите SQL упити.** Првата
верзија го врза наместувањето во самите упити — и учениците сепак не се редеа,
зашто `/api/roster` потоа ги пушта низ `orderPupils` во JavaScript. Значи
наместувањето стигнуваше до базата и не стигнуваше до екранот. `lib/roster-order.ts`
сега е единственото место: читањата остануваат какви што беа, а `arrange`
СТАБИЛНО ги преместува само наместените — ненаместените го задржуваат редоследот
што упитот им го дал и одат по нив.

**И инсталација што намерно заостанува зад кодот не паѓа.** HOME `therapy_dev`
е држена на 32 миграции; `roster_order` таму не постои. Ако наместувањето
живееше во упитот, првото пуштање на новиот код ќе го скршеше `/api/roster` —
а тоа е екранот што се користи секој ден, заради можност што никој уште не ја
употребил. Сега `to_regclass` прашува за објектот што упитот навистина би го
разрешил (истото правило како кај заштитите во миграциите), па список без
табела се чита во редоследот на читачот — точно тоа што „нема наместување"
значи. Запишувањето тоа го кажува со 503 и со името на миграцијата, наместо со
500 за релација што ја нема.

Две работи што ги најде пуштањето, не читањето: двојното редење на учениците
погоре, и **шест тврдења во `podatoci.browser.mjs` што беа застарени
очекувања**, не дефекти — ги читаа броевите како текст на цела ќелија, а таму
сега стојат и стрелките. Сега читаат `.row-number .rn`, а прекинувачот „прикажи
броеви" го крие БРОЈОТ, не ќелијата: стрелките се начинот да се смени
редоследот и не смеат да исчезнат со нумерацијата. Тоа е посебно тврдење.

Проверено: `npm run test:order` (24 тврдења врз училиште што не постои,
вклучувајќи дека ненаместениот новајлија оди на крај, дека еден список не го
менува другиот, дека празно наместување го враќа читачевиот редослед и дека
наместувањето умира со годината), `npm run test:podatoci` (81, во вистински
прелистувач, со читање од `roster_order` по целосно освежување) и целиот
...
## Светла и темна тема: еден избор за целиот пакет (23 Sep 2026)

Сопственикот: „сакам светла и темна тема во сите апликации, на некои имам само
темна". Измерено пред измената, темата беше решена на четири начини: S-Dnevnik и
Fusion имаа свој прекинувач (клуч `theme`), AkciskiPlan свое копче (друг клуч,
`evidence_theme_v1`), шест страници (Настава, НаставаУреди, Податоци,
Преглед-База, Синхронизација, start) само го следеа Windows без прекинувач —
на темен Windows само темна — а работниот простор беше напишан само темен.

**`mtb-theme.js` е единствената копија**, вчитана во `<head>` на секоја страница
од пакетот, пред првото цртање, па страницата не трепнува во другата тема. Таа
само поставува `<html data-theme="light|dark">`; боите ги држи секоја страница.
CSS страниците ја читаат ознаката (`:root[data-theme="dark"]` наместо
`@media (prefers-color-scheme)`); S-Dnevnik, Fusion и AkciskiPlan ја слушаат
`mtb:theme` и си ја ставаат својата класа на body. Без избор се следи системот.

**Клучот е `theme`, со вредностите `dark-mode`/`light-mode`** — тоа што S-Dnevnik
отсекогаш го пишувал. Постојниот избор затоа важи насекаде од првиот ден, а
постара копија на дневникот го чита истото. Изборот од AkciskiPlan се презема
еднаш, само ако уште нема избор за пакетот; стариот клуч само се чита.

**Ништо не се запишува додека некој не избере.** Настава, НаставаУреди и
Податоци ветуваат дека не чуваат ништо во прелистувачот, и тоа останува точно за
податоците; темата е удобност на гледачот, како избраниот сервер.

**Прекинувачот е еден по екран.** Лентата црта 🌙/☀️ само кога страницата нема
свој (`data-mtb-theme-control`) — S-Dnevnik, Fusion и AkciskiPlan си ги
задржуваат своите. Прекинувачот на S-Dnevnik беше `position: absolute` во
однос на СТРАНИЦАТА, па откако заедничката лента дојде над заглавјето, тој
стоеше под неа и **не можеше да се кликне**. Сега е закачен за заглавјето.

**Рамките го следат работниот простор.** Школката ги вчитува апликациите од
избраниот сервер — од GitHub Pages тоа е друг origin со своја меморија — па
изборот им се праќа со `postMessage` (`{type:'mtb:theme'}`), истиот пат и со
истата проверка на испраќачот како фокусот. Рамката го применува, не го
запишува. Заедничката лента останува темна во двете теми, намерно: нејзините
бои на состојбата се мерени врз темна подлога.

Светлата палета на работниот простор е нова (темната е бајт-за-бајт старата,
само именувана); измерени се паровите текст/подлога, сите над 4.5:1.
`mtb-theme.js` е додадена во `public-static.ts` — **серверот мора да се
рестартира** по `git pull`, инаку старата листа ја одбива и страниците остануваат
светли без прекинувач. Тестот во `app-contract.test.ts` тврди дека секоја
страница го вчитува во `<head>` и дека ниту една повторно не го следи системот
сама; проверено дека паѓа кога една страница го нема.
...
**И известување по одделение, зашто така се доставува на хартија.**
„Известувања по одделение" е петтиот приказ и единствениот чија единица е ЛИСТ
наместо мрежа: за секое одделение, целата недела како список — кој ден, кој час,
кое дете, кој кабинет. Тоа е праксата што веќе постои во училиштето, кажана од
сопственикот: секој кабинет му доставува на одделението список по дете, зашто
тоа е податокот што му треба на оној што го држи часот во тој термин. Мрежата
одговара „колку ми недостигаат"; листот одговара „кого, кога и кај кого".

Три работи во него се одлуки, не ситници:

- **Прегрупирање, не второ пресметување.** Ги чита `data.cells` онака како што
  ги чита и „По кабинет" — кој третман на кој час паѓа и за колку минути го
  решава `lib/crossing.ts` и никој друг. Втора аритметика во прелистувачот би
  можела да се разиде за минута и ништо не би кажало.
- **Без минутажа на третманот.** Колку од часот трае третманот е анализа, а не
  порака до одделението — стои во мрежата и во картичката на лебдење, каде што
  се и гледа. Одлука на сопственикот, 12 септември: „тоа е повеќе информација".
- **Одделение без изземања се ИМЕНУВА**, во една реченица под листовите, а не се
  испушта. Тишина што не е кажана изгледа како лист што не се отпечатил.
- **Секој лист се прекршува по страница** во печатење (освен последниот), зашто
  се предава по одделение, а не како една книшка.

Неповрзаните третмани остануваат во својот панел, а под листот на одделението
стои само бројка што покажува кон него: на наставникот не може да му се каже на
кој час е детето ако токму тоа не можело да се одреди.

Приказот бара цела недела како и неделната мрежа, па префрлањето меѓу тие два
не праќа ново барање — истиот одговор, друго цртање. Тестот го тврди тоа во
двете насоки, зашто инаку „бара без ден" би поминало и кога воопшто не прашало.

**Работниот простор сега почнува со процедура (12 Sep 2026).**
`MTB-Workspace.html` е замислен како една врата за целиот систем, а не ја
одговараше првата работа што секој ден се прашува: КОЈ КОМПЈУТЕР Е ВКЛУЧЕН.
Велеше „Избери база во start.html" и те праќаше на друга страница — што значи
дека „едно место од кое се почнува" не беше точно.

Сега скенира сам. Список на адреси, скенирање и памтење се
`configuredServers`, `probeServer` и `rememberServer` во `app-navigation.js` —
ИСТИТЕ што ги користат лентата и `start.html`. Ниту една нова копија од
пронаоѓање сервер; `probe` и `serverName` само беа недостапни однадвор и сега
се објавени.

Однесувањето, по случаи, зашто секој од нив беше причина да се греши:

- послужен ОД сервер → origin-от Е базата, процедурата воопшто не се јавува;
- веќе избран и жив сервер → тишина, не прашува секое утро (процедура што
  прашува кога одговорот е ист е процедура што луѓето ја кликаат без да ја
  читаат);
- точно еден одговара и никој не бирал → се влегува само;
- повеќе одговараат → чека човек, зашто двете машини можат да се викаат исто;
- избраниот е изгаснат → се јавува сама и кажува што да се провери, наместо
  тивко да не работи;
- чипот БАЗА е копче што ја отвора намерно.

**Рамките се вчитуваат од origin-от на ИЗБРАНИОТ сервер.** Отворен од GitHub
Pages, работниот простор инаку ги вгнездува Pages копиите — а кај S-Dnevnik тоа
е тивка стапица: таму автоматската синхронизација е ИСКЛУЧЕНА (`servedByApi`
бара localhost или .ts.net), па страницата работи офлајн засекогаш и изгледа
убедливо. Origin и понатаму значи една база, што е целата причина поради која
`start.html` пренасочува наместо да учи некого да бара.

**`?embed=1` ја гаси заедничката лента во вгнездената копија.** Две редици исти
линкови една врз друга беа токму она што сопственикот го нарече збунувачко. Се
гаси САМО лентата: проверката на здравјето, сесијата, потпишаниот `fetch` и
изборот на сервер остануваат истиот модул. И чипот БАЗА во школката паѓа на
името на машината кога инсталацијата нема своја ознака — „БАЗА: —" на жива
врска се чита како дефект.

Проверено во прелистувач со пресретнати адреси (`assisstant.github.io` и
tailnet имињата свртени кон стуб): 18 тврдења низ сите шест случаи, вклучувајќи
дека вгнездената апликација не црта втора лента и дека изгасната машина не се
кликнува.

**Одделението доби сопственик (12 Sep 2026).**
`student_enrollments.oddelenie` постоеше од 11 септември, беше именувано во
миграција 031 како сопственик на опсегот генерации — и **ништо не можеше да го
запише**. Беше додадено рачно на една машина и се читаше само од
`/api/teaching/subjects`. Факт со читател а без запишувач е факт што застарува
првиот пат кога дете ќе премине во повисоко одделение.

Сега `PATCH /api/students/:publicId` го прима, `/api/roster` го враќа, а
работниот простор го уредува. Секој од трите беше потребен: без враќање во
`roster` ниту еден екран не може да го ПОКАЖЕ, па не може ни да го смени.

**Двете полиња стојат едно до друго и се именувани со тоа што значат** —
„Паралелка · каде учи" и „Одделение · која генерација". Кај единаесет
паралелки се исти, кај петте комбинирани не се: дете во „II комбинирана 2,3,4"
е во ТРЕТО одделение и учи во ВТОРАТА комбинирана паралелка. Кога ќе се
разидат, страницата пишува белешка, не предупредување — кај комбинирана
паралелка разидувањето е ТОЧНО, и предупредување што се јавува кога сè е во
ред е предупредување што луѓето учат да го игнорираат. Белешката се менува
додека се бира, не дури по зачувување.

Во списокот одделението се пишува САМО кога кажува нешто ново („II комб. ·
одд. III"). Кога се совпаѓа со римскиот број во ознаката, секој ред би носел
двапати иста работа и разликата што е важна би се изгубила во неа.

**Записот е подреден по СОПСТВЕНИК НА ФАКТОТ, не по редослед на полиња.** Горе
е она што важи за човекот без разлика на годината (име, статус); во посебна
рамка е она што е факт за ОВАА учебна година — паралелка, одделение и кај кои
терапевти е детето, со годината напишана во насловот. Тоа е истата поделба
поради која `student_enrollments` воопшто постои, и кога екранот ја покажува,
никој не се прашува зошто менувањето паралелка не ја менува архивската година.
Чиповите со терапевтите се копчиња: водат кон тој список и го осветлуваат во
распоредот.

Зачувувањето не праќа ништо кога нема промена и го кажува тоа, како
каталогот — барање што не менува ништо сепак враќа одговор што екранот го
применува, а тоа е како се губи корекција направена во меѓувреме.

**Предметот и одделенијата на наставникот (12 Sep 2026).**
Двете полиња што беа најслаби на тој екран. „Предмет" беше празно поле за
пишување, иако каталогот на МОН веќе стои во `teaching_subjects` и веќе се нуди
во НаставаУреди. Сега е избирач од истиот каталог — најпрво `datalist`, истиот
ден заменет со видливите чипови од „Еден наставник, повеќе предмети" подолу.
**Понуда, не ограничување**: `teachers.subject` останува слободен текст, зашто
работната книга на училиштето пишува кратенки (`ФЗО.`) што серверот ги разбира
преку `SUBJECT_ALIASES`, и еден избор од мени не смее тивко да ги замени.
Зачуваното што каталогот не го знае се задржува, за да не изгледа како грешка
што треба да се поправи.

**И се затвори тивко бришење.** `sync()` го празнеше полето штом улогата е
„одделенски" — при СЕКОЕ отворање, не само при менување. Значи: отвораш
одделенски наставник на кого работната книга му запишала предмет, полето е
празно и заклучено, притискаш „Зачувај" за нешто сосема друго, и предметот е
избришан од базата. Иста форма како препишаното спелување на име во истата
школка. Сега полето го покажува зачуваното и кога е заклучено, а зачувувањето
одлучува намерно: **предметен** → она што пишува; **станал одделенски сега** →
`null`, и белешката го кажува тоа однапред; **веќе бил одделенски** → полето
воопшто не се праќа. Серверот ги разликува отсутно и `null` уште од почеток
(`PUT /api/teaching/teacher/:id` гради `SET` список наместо `COALESCE`) — само
никој не ја користеше таа разлика.

**Одделенијата: улогата се гледа само кај чекнатите**, а над решетката стои
реченица во живо — „Раководител: I-а · Предава во: II-б, III". Осумнаесет
паралелки со по еден избирач секоја беа ѕид низ кој не се гледа што е избрано;
сега избраното се чита без броење. И пишува дека списокот се запишува како
ЦЕЛИНА, зашто тоа е вистина за овој endpoint и не е вистина за ќелија во
распоред — двете правила стојат едно до друго во овој проект и нивното мешање
е грешка што чини недела часови.

**Едно избрано лице, низ сите прозорци (12 Sep 2026).**
Списокот покрај распоредот во `MTB-Workspace.html` досега не значеше ништо за
прозорецот до него — а тоа е прашањето што се поставува: го избирам ова дете,
КАДЕ Е ТОА во неделата. Сега изборот се праќа во секоја рамка и се осветлува:
термините на детето, колоната на терапевтот, часовите на наставникот во
вкрстувањето. Повторен клик гасне — осветлувањето е поглед, не режим.

Три работи во него се одлуки:
...
- **Страницата само ОБЕЛЕЖУВА, осветлувањето е една копија.** Секој екран става
  `data-focus` на своите елементи (`applyFocus` се повикува по секое
  прецртување, зашто нова мрежа значи нови елементи); самото осветлување е во
  `app-navigation.js`. Две копии би можеле да осветлат различно за исто дете.
- **Она што не е избрано се ПОВЛЕКУВА, не се крие.** Празен термин и туѓ час се
  ...
  крие е распоред што лаже. Тестот го тврди: ниту еден елемент не добива
  `display: none`.

**Списокот на клучеви е JSON, не низа делена со празно место.** Очигледната
форма е погрешна: `public_id` е САМО ОБИЧНО во облик `RS-…`, и еден што носи
празно место се делеше на два клуча што не совпаѓаат со ништо — осветлувањето
тивко не се јавуваше, што е најтешкиот вид грешка зашто ништо не поцрвенува.

**`Nastava.html` доби свој таб, а стариот „Настава" се вика „Уреди настава".**
Истиот збор значеше две различни страници во заедничката лента и во школката.

**А каталогот на терапевтот веќе се уредуваше од конзолата.** Претходната
верзија на `docs/PLAN-rabotna-konzola.md` тврдеше спротивно; проверено во
прелистувач, изборот на терапевт отвора список со чекбоксови и „Зачувај список"
праќа по едно `PUT`/`DELETE` за секоја промена. Документот е исправен.

**Демо распоред, за да има што да се гледа (12 Sep 2026).**
`npm run demo:teaching` генерира распоред на настава од правилата што
сопственикот ги кажа. Постои зашто вкрстувањето, неделната мрежа, известувањата
и картичката во кабинетите се ПРАЗНИ додека нема ниту еден час — а да се
искуцаат 400 ќелии за да се види дали еден екран се чита добро не е работа за
едно попладне.

Правилата, како што беа кажани: предметите одат по важност (македонски,
математика, па другите) од првиот до последниот час; фондот е 21–23 часа
неделно, а демото користи 21; во ОДДЕЛЕНСКА наставникот ги држи сите часови
освен физичко, кое оди на придружен предметен наставник; во ПРЕДМЕТНА
наставникот СМЕЕ да предава во сите одделенија — не мора — и кога ќе го наполни
фондот, друг наставник се ангажира.

Три работи во него се одбрана од самото него:

- **Не создава никого.** Час што никој не е слободен (или под фондот) да го
  земе се запишува БЕЗ наставник и се брои во извештајот. Измислено име би било
  човек што ова училиште не го вработува — а „друг наставник се ангажира" е
  одлука на човек, не на програма.
- **Одделенска не се погодува од ознаката.** Се чита `teachers.kind` за
  одделенскиот раководител на таа година: комбинирана паралелка нема римски број
  од кој би се погодувало, а `kind` веќе точно го значи тоа.
- **Опсегот на генерации доаѓа од `student_enrollments.oddelenie`** (миграција
  031 го именуваше за сопственик), па ознаката на паралелката, па ништо —
  паралелка без читлив опсег се пријавува и останува празна. Измислено
  одделение би измислило наставен план.

**Пуштено против вистинска база, и тоа најде две работи (12 Sep 2026).**
Досега генераторот беше проверен само со 17 чисти тврдења. Скриптата околу
него — нејзиниот SQL — не беше извршена ниту еднаш. Кренат е PostgreSQL 16 со
`C.utf8`, применети се сите 32 миграции на празна база, посеано е измислено
училиште (17 паралелки, 22 наставници, 11 раководства) и скриптата е пуштена.

- **Првото барање беше мртво.** `WHERE b.kind = 'nastava-am'` — колоната се
  вика `schedule`; и `coalesce(o.ordinal, …)` — `bell_period_overrides` нема
  `ordinal` воопшто (менува ознака, почеток и траење, никогаш редослед). Два
  непостоечки идентификатора во еден упит: скриптата ќе умреше во првата
  секунда на неговиот компјутер.
- **Часовите се собираа од СИТЕ ШЕСТ плана.** Измерено: I одделение е **22
  часа** под кој било поединечен план — точно 21–23 што сопственикот ги кажа —
  и **35 часа** како унија, зашто предметите што постојат само во посебните
  планови се редат врз редовниот. Првото пуштање даде одделенски наставници со
  33–35 часа и изгледаше како проблем со распределбата; беше наставен план што
  никој не го предава.

Унијата е ТОЧНА за менито (`/api/teaching/subjects` нуди избор, а сокриен
предмет е ѕид) и погрешна за НЕДЕЛА. Кој план ја следи една паралелка не се
изведува — миграција 031 вели дека ништо не смее да пресметува од описот — па
демото изречно претпоставува: `--plan`, стандардно „Редовен наставен план", и
го пишува во извештајот.

**И бројката во извештајот беше преувеличена.** „Најгусто: математика во 16
паралелки одеднаш" броеше ПАРАЛЕЛКИ; единаесет од нив се одделенски, каде
наставникот веќе го држи часот и не се натпреварува со никого. Сега брои
НАСТАВНИЦИ што планот навистина ги употребил — 10 наместо 16. Бројка што го
преувеличува проблемот е бројка по која никој не постапува.

По поправките: 410 часа, 17 паралелки, 22 наставници, **0 без наставник**,
фонд 20–25 кај одделенските и 21 кај предметните-генералисти, 7–10 кај
специјалистите — обликот што сопственикот го опиша. `day_order` е точен 1–5,
а `/api/teaching/crossing` ги прочита сите 410 без ниту еден час вон списокот.

**Преработено по вистинскиот распоред на училиштето (12 Sep 2026).** Сопственикот
испрати слика од работната книга и рече да се работи по неа, а не по строгото
правило. Три работи се променија, и сите три беа грешки:

- **Паралелките не почнуваат сите со ист предмет.** Првата верзија делеше од
  понеделник, најважниот предмет прв, за секоја паралелка — па цело училиште ја
  отвораше неделата со македонски истовремено и требаше по еден наставник по
  македонски за секоја паралелка. Работната книга не изгледа така: една почнува
  со мак., друга со одд., трета со ж.в. Сега делењето почнува од ДРУГ ДЕН за
  секоја паралелка, а кој од двата најтешки предмети го отвора денот се менува —
  како што работната книга има мак. во понеделник и мат. во вторник во иста
  паралелка. Петчасовен предмет и понатаму паѓа на пет различни дена.
- **Кратенките во колоната „Предмет" не се совпаѓаа со каталогот.** Базата пишува
  `ФЗО.`, каталогот на МОН пишува „Физичко и здравствено образование". Споредени
  како обичен текст двете никогаш не се среќаваат, па единствениот наставник што
  навистина држи само физичко беше прескокнат за секој час по физичко во
  училиштето, а фондот падна врз оној што случајно бил најлесен. Изгледаше како
  распределба, а беше промашување. `SUBJECT_ALIASES` ги врзува кратенките што
  реално се користат; непозната кратенка паѓа назад на обична споредба.
- **Специјалист не се става на туѓ предмет.** „Силвана е само англиски, Драган е
  само физичко." Наставник БЕЗ запишан предмет сега доаѓа пред запишан
  специјалист за се' друго — специјалистот се зема само ако нема кој друг, зашто
  час без наставник не помага никому.

**Физичко во одделенска сега го држи одделенскиот наставник**, како што пишува во
работната книга: таму фзо. стои во редот на паралелката. Придружниот наставник е
покрај одделенскиот, не наместо него, а генериран распоред не смее да тврди
кадровски аранжман што изворот не го покажува. `--fzo-pred` го префрла, ако
треба.

Пробно по правило, `--apply` за да запише, и **одбива година што веќе има
часови освен со `--replace`**: демо што тивко заменува вистински распоред е
точно она што овој проект постојано го плаќа. Се брише со „Испразни ја годината"
во НаставаУреди.

Извештајот кажува и една вистина за самото правило: строгиот редослед по важност
става македонски на прв час во СИТЕ паралелки истовремено, па толку наставници по
македонски се потребни во тој момент. Вистински распоред тоа го омекнува со
разместување по денови — генераторот намерно не го прави тоа, зашто тогаш би
престанал да одговара на прашањето што е поставено.

`test/teaching-demo.test.ts` е 17 тврдења врз училиште што не постои (без сервер,
без база): дека првиот час од денот е најважниот предмет, дека петчасовен предмет
се рашири по деновите, дека никој не е закажан на две места истовремено, дека
фондот е фонд, дека непокриеното се пријавува наместо да се измисли, и дека ист
влез секогаш дава иста недела.

**И начин да се погледне, па да се врати како што било (12 Sep 2026).**
Сопственикот побара да го види демото на машина што го држи вистинскиот
распоред. „Испразни ја годината" не е враќање: демото со `--replace` ги БРИШЕ
постоечките часа пред да ги запише своите, а празнењето потоа остава празна
година, не вчерашната.

`npm run timetable:snapshot` ја зема годината в датотека и `timetable:restore`
ја враќа истата. **Само `lessons`, и само една година** — тоа не е
поедноставување туку измерено: `demo-timetable.ts` не пишува ништо друго, па
снимка од повеќе би тврдела дека штити нешто што не било во опасност, а
враќањето би било слободно да поништи туѓа корекција направена в меѓувреме.

Id-то не се чува, зашто ништо во оваа шема не покажува кон час. Скриптата тоа
не го зема на доверба: на секое враќање го прашува `pg_constraint` и одбива ако
нешто почнало да се повикува на `lessons` — истата постапка како кај
`roster-purge.ts`, и од иста причина. Тој ден id-то Е идентитет и враќање по
содржина повеќе не е целосно враќање.

**Празна снимка е дозволена; скратена не е.** Тоа е разликата што одлучува дали
враќањето е враќање или бришење: година што навистина била празна пред демото е
чест случај и точно за него ова постои, а датотека на која ѝ недостига половина
изгледа исто. Затоа снимката го носи бројот покрај редовите и се одбива кога
двата не се совпаѓаат — правило 3 применето таму каде што може да се провери,
наместо на слепо.

Пробно по правило. Враќањето е една трансакција и се откажува ако не влезат
точно онолку редови колку што има снимката: половина распоред е полош од двата
други исхода.

Пуштено против вистинска база, зашто демото веќе покажа дека чисти тврдења не
го фаќаат SQL-от: 14 рачно внесени часа (со еден без наставник и еден без
предмет) → снимка → демо со `--replace` што ги избриша → враќање → **ист md5 на
содржината**. Плус празна година в двете насоки, и четирите одбивања проверени
дека навистина одбиваат: скратена датотека, паралелка што ја нема, туѓ клуч кон
`lessons`, и несовпаднат број.

**Картичката на лебдење се гасне (12 Sep 2026).** И во Настава („Картичка на
лебдење") и во распоредот на кабинетите („📍 Каде е детето"). Побарано од
сопственикот, и точно затоа што ништо не зависи САМО од лебдењето: панелот на
клик во Настава и мрежата остануваат, па гаснењето не крие ниту еден одговор —
само тишина додека се чита. Во Настава изборот е во променлива (страницата
намерно не чува ништо во прелистувачот); во Fusion се памти во
`mtb_fusion_where_v1`, како и начинот на приказ, зашто таа страница и онака
памти избори.

**Двете насоки, до крај (12 Sep 2026).** Сопственикот ги именуваше двете
половини на истото прашање. Едната беше одговорена; двете дупки што останаа се
затворени со ист одговор на API-то, ниту една со втора пресметка.

**Во распоредот на кабинетите: каде е детето во тој термин.**
`RasporediFusion.html` секогаш знаеше дека терапевтот го зема детето во 09:40 и
никогаш од што. Лебдењето над термин сега кажува од кој час, кое одделение и кај
кој наставник, и колку минути од тој час.

Тоа бараше едно проширување на `/api/teaching/crossing`, и тоа е суштината:
секое отсуство сега носи `studentPublicId` и `slots` — СУРОВИТЕ `time_slot`
стрингови од кои таа сесија е составена. Распоредот на кабинетите ги држи токму
тие стрингови, па својата ќелија ја наоѓа со обична еднаквост на низи, без
втора копија од ѕвоната и без втора копија од аритметиката. Совпаѓање по ИМЕ
беше единствената алтернатива и е забрането: две деца во ова училиште делат
едно име (правило 2).

Истото поправи два тивки собирања во самото вкрстување: клучот на сесијата беше
`ден|терапевт|ИМЕ`, па два соименици кај ист терапевт во ист ден се спојуваа во
ЕДНА сесија, а клучот во часот беше `ИМЕ|терапевт`, па вториот соименик паѓаше
од часот од кој навистина е изземен. И двата сега одат по `public_id`.

Вчитувањето е ОДДЕЛНО и не се чека: училиште без внесена настава, постар сервер
без овој одговор или бавна мрежа не смеат да го задржат распоредот на кој се
работи. Нема одговор → нема картичка, и ништо друго не се менува. Термин што
воопшто не може да се врзе за час го кажува тоа со причината — тоа е токму
случајот кога човекот мора да дознае дека детето не може да се лоцира.

**Во мрежата на наставата: кој кабинет кое дете зема.** Чекбоксот „Испиши кој
кого зема" ги претвора ќелиите од број во список — важи во „По одделение", „По
наставник" и „Цела недела", а „По кабинет" и онака ги пишува имињата. Бројот
ОСТАНУВА покрај имињата: мрежата се чита попреку за „каде е густо", а тоа е
бројка, не список.

Држено е во обична променлива, НЕ во `localStorage`: оваа страница намерно не
чува ништо во прелистувачот. И е само друго цртање на истиот одговор — тестот
тврди дека чекирањето не праќа ново барање, зашто бројот и списокот што доаѓаат
од две различни читања се разидуваат точно кога никој не гледа.
