...
async function main() {
    ...
    /**
     * A FILE import restores everything, marker or no marker.
     *
     * `_meta.rowWrites` tells the API "this collection has its own endpoint
     * now, so a whole-document save must not decide it" — a rule about live
     * saves from a running app. Importing a file is the opposite situation: it
     * is the escape hatch of rule 4, the thing someone reaches for when a
     * rewrite has gone wrong mid-year, and it must put back what the file
     * holds. Honouring the marker here would silently skip attendance in
     * exactly the moment the attendance is what is being rescued.
     */
    const sdnForImport = sdnDoc && (sdnDoc as any)._meta?.rowWrites
        ? { ...(sdnDoc as any), _meta: { ...(sdnDoc as any)._meta, rowWrites: undefined } }
        : sdnDoc;
    if (sdnForImport !== sdnDoc) {
        console.log('\nNote: this file announces per-row writes. A file import restores everything anyway.');
    }

    const client = await pool.connect();
    try {
        await client.query('BEGIN');
        await writeAll(client, canonical, base, sdnForImport, report);
