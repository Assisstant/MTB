...
export async function readOrganization(query: QueryFn): Promise<OrganizationInput> {
    const run = async (sql: string, params: unknown[] = []) => {
        try {
            ...
            throw new OrganizationReadError(error instanceof Error ? error.message : String(error));
        }
    };
    ...
    const capabilities: OrganizationCapabilities = {
        ...
        profileEmployeeId: Boolean(probe.teacher_employee_id && probe.therapist_employee_id),
