...
(function () {
  ...
  const topEdge = () => {
    ...
    return nav ? Math.max(EDGE, Math.ceil(nav.getBoundingClientRect().bottom) + EDGE) : EDGE;
  };
  ...
  const ensureInViewport = (animate) => {
    ...
    const ny = clamp(state.y, minTop, Math.max(minTop, window.innerHeight - h - EDGE));
  ...
  tab.addEventListener("pointermove", (ev) => {
    ...
    const minTop = topEdge();
    const newTop = clamp(origTop + dy, minTop, Math.max(minTop, window.innerHeight - dock.offsetHeight - EDGE));
  ...
  window.addEventListener("mtb:navigation-mounted", () => ensureInViewport(true));
